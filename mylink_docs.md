# Dokumentasi Teknis Fitur MyLink Link Terpusat - Jalanin

Dokumen ini merangkum arsitektur, basis data, API, logika validasi, dan integrasi antarmuka untuk fitur **Pustaka Link MyLink Terpusat** & **Validasi Domain Dinamis** pada platform Jalanin.
...
* **`MyLink`**: Tempat penyimpanan link mylink terpusat milik masing-masing pengguna.
* **`MyLinkWhitelistDomain`**: Daftar pola domain mitra resmi yang diizinkan oleh sistem (dikelola oleh Admin).
* **`Activity`**: Menghubungkan aktivitas pada rute perjalanan (*itinerary*) dengan link mylink yang terdaftar di pustaka.
...
model MyLink {
  ...
  actualUrl String     // URL tujuan mylink asli
...
model Activity {
  ...
  myLinkId        String?
  myLink          MyLink? @relation(fields: [myLinkId], references: [id], onDelete: SetNull)
...
## 2. Sistem Validasi Domain Dinamis (`lib/mylink-validator.ts`)

Untuk mencegah penyalahgunaan link eksternal yang tidak sah, sistem melakukan validasi dinamis terhadap setiap input URL mylink menggunakan daftar pola domain yang tersimpan di basis data (`MyLinkWhitelistDomain`).
...
1. **`validateMyLinkUrl(url: string)`**:
   - Memparse URL input.
...
### A. Manajemen Link MyLink (`app/api/mylink/links/route.ts`)
Rute ini diamankan menggunakan pemeriksaan sesi pengguna (`getCurrentUser()`).

* **`GET`**: Mengambil daftar link mylink milik user yang sedang aktif.
...
* **`POST`**: Mendaftarkan link mylink baru ke pustaka.
  - Melakukan validasi URL menggunakan `validateMyLinkUrl`.
  - Mendeteksi provider secara otomatis menggunakan `detectProvider`.
* **`PUT`**: Memperbarui informasi label atau URL link mylink yang sudah ada.
  - Melakukan validasi ulang sebelum memperbarui database.
* **`DELETE`**: Menghapus link mylink.
  - Seluruh aktivitas yang menunjuk ke link ini akan otomatis diset menjadi `null` pada kolom `myLinkId` (tidak memicu penghapusan aktivitas).

### B. Whitelist Mitra Resmi (`app/api/mylink/whitelist/route.ts`)
...
1. Sistem **mempertahankan** referensi `myLinkId` asli pada setiap aktivitas yang disalin.
...
3. Pengklon dapat memperbarui tautan tersebut ke link mylink mereka sendiri secara manual kapan saja lewat halaman pengeditan rute.

---

## 5. Komponen & Alur Kerja Antarmuka (UI/UX)

### A. Halaman Pustaka Link (`app/settings/mylink/page.tsx`)
Halaman pengaturan terpadu bagi kreator untuk mengelola pustaka mylink mereka:
1. **Form Input**: Menambah atau mengedit alias label dan URL mylink.
...
- Pada card input aktivitas, tersedia menu pilihan (*dropdown*) untuk menyematkan link mylink dari pustaka terdaftar.
...
Pada detail itinerary, aktivitas yang memiliki tautan mylink akan merender tombol CTA menarik dengan ketentuan keamanan:
